...
def perform_alignment(seq):
  ...
  for file in files:
    ...
    logger.info("Consensus sequence derived and stored.")
    structure = derive_structure(prefix)
    #logger.info("Saving the final alignment in STOCKHOLM format")
    #rite_final_alignment(virusAligner.refinedAlignment, structure, prefix)
...
def derive_structure(prefix):
  ...
  logger.info("Scanning consensus sequence for local substructures.")
